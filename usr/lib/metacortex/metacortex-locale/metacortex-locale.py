...
class MetacortexLocale:
    ...
    def __init__(self):
        ...
        self.builder.set_translation_domain("metacortex-locale")
        self.builder.add_from_file('/usr/share/metacortex/metacortex-locale/metacortex-locale.ui')
    ...
    def build_lang_list(self):
        ...
        self.countries = {}
        with codecs.open('/usr/share/metacortex/metacortex-locale/countries', "r", encoding='utf-8') as file:
            for line in file:
                line = line.strip()
                split = line.split("=")
                if len(split) == 2:
        ...
        self.languages = {}
        with codecs.open('/usr/share/metacortex/metacortex-locale/languages', "r", encoding='utf-8') as file:
            for line in file:
                line = line.strip()
                split = line.split("=")
                if len(split) == 2:
    ...
    def set_pam_environment(self):
        ...
        if os.path.exists(self.pam_environment_path):
            ...
            if ("XDG_SEAT_PATH" in os.environ):
                # LightDM
                os.system("sed -e 's/$locale/%s/g' -e 's/$shortlocale/%s/g' -e 's/$region/%s/g' -e 's/$time/%s/g' /usr/share/metacortex/metacortex-locale/templates/lightdm_pam_environment.template > %s" % (self.current_language, shortlocale, self.current_region, self.current_time, self.pam_environment_path))
            else:
                # MDM
                os.system("sed -e 's/$locale/%s/g' -e 's/$region/%s/g' -e 's/$time/%s/g' /usr/share/metacortex/metacortex-locale/templates/mdm_pam_environment.template > %s" % (self.current_language, self.current_region, self.current_time, self.pam_environment_path))
